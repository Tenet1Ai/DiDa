...
AVAudioPlayerDelegate, UIGestureRecognizerDelegate,
...
- (NSFetchedResultsController *)fetchedResultsController {
    ...
    if (_fetchedResultsController == nil) {
        ...
        NSDate *toDate = nil;
        if (daySelected == NO) {
            NSUInteger dayOfWeek = [selectedDate dayOfWeek];
            fromDate = [selectedDate fs_dateBySubtractingDays:dayOfWeek];
            toDate = [selectedDate fs_dateByAddingDays:(7 - dayOfWeek)];
        } else {
...
- (void)viewDidLoad {
    ...
    calendar.calendarAppearance.calendar.firstWeekday = 1; // Sunday == 1, Saturday == 7
    ...
    calendar.calendarAppearance.dayCircleColorToday = kPink;
    calendar.calendarAppearance.dayCircleColorSelected = kBlue;
    calendar.calendarAppearance.weekDayTextColor = kBlueText;
...
- (void)calendarDidDateSelected:(JTCalendar *)calendar date:(NSDate *)date {
    ...
    NSString *dateString = [date fs_stringWithFormat:@"d MMMM yyyy"];
    self.navigationItem.title = dateString;
}

- (void)calendarDidLoadPreviousPage {
//    DLog(@"%@", calendar.currentDate);
    NSDate *date = calendar.currentDate;
    selectedDate = date;
    _fetchedResultsController = nil;
    [mainTableView reloadData];
    [calendar reloadData];
    NSString *dateString = [date fs_stringWithFormat:@"'Week' W 'of' MMMM"];
    self.navigationItem.title = dateString;
}
